  ...
  <form @submit.prevent="searchData" class="search-form">
    <div class="search-container">
      ...
      <div class="select-container">
        <select v-model="selectedCriteria" id="misel">
          <option value="PCID">PubChem ID</option>
          <option value="CompoundName">Nombre del compuesto</option>
          <option value="PlantName">Nombre de la planta</option>
...
export default {
  ...
  methods: {
    searchData() {
      ...
      const misel = document.getElementById ("misel").value;
      

      const search = this.$normalize(this.search.toLowerCase());
      this.results = data.filter(item => {
        let name = '';
        if(misel=="CompoundName") {
          name = this.$normalize(item.CompoundName.toLocaleLowerCase());
        }
        else if(misel=="PlantName") {
          name = this.$normalize(item.PlantName.toLocaleLowerCase());}

         else if(misel=="Organism_type") {
          //name = this.$normalize(item.literatures[0].organism_type.toLocaleLowerCase());
          const literatura = item.literatures 
          literatura.forEach(element => {name = this.$normalize(element.organism_type.toLocaleLowerCase())
